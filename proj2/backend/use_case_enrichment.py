...
def enrich_use_case(use_case: dict, original_text: str) -> dict:
    ...
    if not main_flow or len(main_flow) < 4:
        ...
        action_verbs = ['view', 'search', 'add', 'create', 'update', 'delete', 
                       'download', 'upload', 'login', 'register', 'pay', 'submit',
                       'confirm', 'verify', 'select', 'filter', 'manage', 'configure']
        
        action = next((verb for verb in action_verbs if verb in title_lower), "perform action")
        ...
        if main_flow:
            ...
            for step in comprehensive_flow:
                ...
                if not any(step.lower()[:20] in str(existing).lower()[:20] 
...
def should_merge_use_cases(uc1: dict, uc2: dict) -> bool:
    ...
    if any(word in words2 for word in ['filter', 'sort', 'refine']) and 'search' in words1:
        return True
    ...
    if actor1 == actor2:
        ...
        has_related_action = any(
            action in title1 and action in title2 
...
def merge_use_cases(uc1: dict, uc2: dict) -> dict:
    ...
    merged["preconditions"] = list(set(
        merged.get("preconditions", []) + uc2.get("preconditions", [])
    ))
    # Remove generic placeholders if we have real content
    if len(merged["preconditions"]) > 1:
        merged["preconditions"] = [p for p in merged["preconditions"] if p != "No preconditions"]
    ...
    # Merge outcomes
    merged["outcomes"] = list(set(
        merged.get("outcomes", []) + uc2.get("outcomes", [])
    ))
    if len(merged["outcomes"]) > 1:
        merged["outcomes"] = [o for o in merged["outcomes"] if o != "No outcomes"]
    
    # Merge stakeholders
    merged["stakeholders"] = list(set(
        merged.get("stakeholders", []) + uc2.get("stakeholders", [])
    ))
    if len(merged["stakeholders"]) > 1:
        merged["stakeholders"] = [s for s in merged["stakeholders"] if s != "No stakeholders"]
...
def normalize_use_case(use_case: dict) -> dict:
    ...
    for field in ["preconditions", "sub_flows", "alternate_flows", "outcomes", "stakeholders"]:
