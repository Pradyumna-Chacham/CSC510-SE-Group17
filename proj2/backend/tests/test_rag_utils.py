import pytest
import os
from rag_utils import process_document, extract_use_cases, semantic_chunk
...
async def test_process_document_extraction(complex_requirements_doc):
    ...
    with patch('rag_utils.get_llm_response') as mock_llm:
        ...
        assert any("encryption" in req.lower() for req in use_case["security_requirements"])
...
async def test_requirement_quality_analysis(ambiguous_requirements_doc):
    ...
    with patch('rag_utils.get_llm_response') as mock_llm:
        ...
        assert any("metrics" in sugg.lower() for sugg in analysis["improvement_suggestions"])
        assert any("security" in sugg.lower() for sugg in analysis["improvement_suggestions"])
            ...
            "use_cases": [
                {
                    ...
                    "steps": ["Open customer profile", "View order history", "Filter by date"]
                },
                {
                    "id": "UC2",
                    ...
                    "steps": ["Access shipping dashboard", "View active deliveries", "Check status updates"]
                }
            ]
        }
            ...
            "use_cases": [
                {
                    ...
                    "processed_requirements": ["Secure authentication", "Password rules"]
                }
            ]
        }
