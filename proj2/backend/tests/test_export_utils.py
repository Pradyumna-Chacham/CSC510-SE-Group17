...
def test_export_to_json(sample_use_case, sample_session_context):
    ...
    assert result["metadata"]["project_context"] == sample_session_context["project_context"]
...
def test_jira_description_with_missing_fields():
    ...
    assert len(description) == 0  # Should return empty string when no required fields exist
