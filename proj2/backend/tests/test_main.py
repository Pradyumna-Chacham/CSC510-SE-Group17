...
import pytest
from main import (
    app, UseCaseEstimator, get_smart_max_use_cases, get_smart_token_budget,
    clean_llm_json, flatten_use_case, ensure_string_list, compute_usecase_embedding,
    extract_use_cases_batch, parse_large_document_chunked, generate_session_title,
...
SAMPLE_USE_CASE = {
    ...
    "main_flow": ["User enters credentials", "System validates", "User is authenticated"],
...
class TestUseCaseEstimator:
    ...
    def test_estimate_use_cases_compound(self):
        ...
        assert details["conjunction_action_count"] >= 2, "Should count actions split by 'and'"
        assert details["found_actions"].issuperset({"login", "add"}), "Should detect both actions"
...
class TestSmartTokenBudget:
    def test_get_smart_token_budget_basic(self):
        ...
        assert budget == min(1200, max(300, base_tokens)), "Should calculate budget correctly"
...
class TestAPIEndpoints:
    ...
    def test_update_session(self, client):
        ...
        # Update session
        response = client.post("/session/update", json={
    ...
    def test_parse_use_case_fast(self, mock_extract, client):
        ...
        response = client.post("/parse_use_case_rag/", json={
    ...
    def test_use_case_refinement(self, mock_embedder, client):
        ...
        with patch('main.extract_use_cases_single_stage') as mock_extract:
            mock_extract.return_value = [SAMPLE_USE_CASE]
            client.post("/parse_use_case_rag/", json={
                "raw_text": "User logs in",
                "session_id": session_id
            })
        
        # Get use case ID from database
        use_cases = client.get(f"/session/{session_id}/history").json()["generated_use_cases"]
        use_case_id = use_cases[0]["id"]

        # Test refinement - more main flows
        response1 = client.post("/use-case/refine", json={
            "use_case_id": use_case_id,
            "refinement_type": "more_main_flows"
        })
        assert response1.status_code == 200
        data1 = response1.json()
        assert "refined_use_case" in data1
        assert len(data1["refined_use_case"]["main_flow"]) >= len(SAMPLE_USE_CASE["main_flow"])

        # Test refinement - more sub flows
        response2 = client.post("/use-case/refine", json={
            "use_case_id": use_case_id,
            "refinement_type": "more_sub_flows"
        })
        assert response2.status_code == 200
        data2 = response2.json()
        assert len(data2["refined_use_case"]["sub_flows"]) >= len(SAMPLE_USE_CASE["sub_flows"])

        # Test refinement - custom instruction
        response3 = client.post("/use-case/refine", json={
            "use_case_id": use_case_id,
        ...
        has_security = any("security" in str(item).lower() 
                         for items in [refined_use_case["main_flow"], 
    ...
    def test_document_parsing(self, mock_extract, client):
        ...
        with patch('main.parse_large_document_chunked') as mock_parse:
            ...
            response = client.post(
                f"/parse_use_case_document/?session_id={session_id}",
                files=files
            )
    ...
    def test_session_management(self, client):
        ...
        session_with_text = client.post("/parse_use_case_rag/", json={
        ...
        list_response = client.get("/sessions/")
        sessions = list_response.json()["sessions"]
        assert not any(s["session_id"] in [session_id_1, session_id_2] for s in sessions)
    ...
    def test_error_handling(self, client):
        ...
        response = client.post("/use-case/refine", json={
            "use_case_id": 99999,
            "refinement_type": "more_main_flows"
        })
    ...
    def test_chunked_processing(self, mock_extract, client):
        ...
        response = client.post("/parse_use_case_rag/", json={
            "raw_text": long_text,
            "project_context": "Large Document Test"
        })
...
class TestProcessing:
    @patch('main.extract_use_cases_single_stage')
    @patch('main.chunker')
    def test_document_chunking(self, mock_chunker, mock_extract, client, mock_pipe_response):
        ...
        mock_chunker.chunk_document.return_value = [
            {"text": "User can login and view profile", "chunk_id": "1", "char_count": 1000},
            {"text": "User can edit settings and manage account", "chunk_id": "2", "char_count": 1000},
            {"text": "User can search products and place orders", "chunk_id": "3", "char_count": 1000}
        ]
    ...
    def test_duplicate_detection(self, mock_embedder, client, mock_pipe_response):
        ...
        with patch('main.extract_use_cases_single_stage') as mock_extract:
            ...
            # First submission
            response1 = client.post("/parse_use_case_rag/", json={
                "raw_text": "User logs in",
                "session_id": session_id
            })
            
            # Second submission of similar use case
            response2 = client.post("/parse_use_case_rag/", json={
                "raw_text": "User logs into system",
                "session_id": session_id
            })
